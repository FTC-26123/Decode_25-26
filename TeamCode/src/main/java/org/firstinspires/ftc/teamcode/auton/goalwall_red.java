...
@Autonomous(name = "GoalWall_RED_TIME")
public class goalwall_red extends LinearOpMode {

    final int apriltag_height = 25;
    final double limelightMountAngleDegrees = 35;
    final double limelightLensHeightInches = 16.5;
    double ty = 0;
    double distance = 0;
    ...
    // --- AUTON SEQUENCE (with PID calls restored) ---
    @Override
    public void runOpMode() throws InterruptedException {
        ...
        PID_forward(44, 0.5);        // Replaced driveForward
        sleep(500);

        PID_rotateRight(90, 0.5);    // Replaced rotateRight
        sleep(500);
        ...
        PID_rotateLeft(30, 0.40);    // Replaced rotateLeft
        sleep(500);

        intake.setPower(1);
        PID_forward(30, 0.75);       // Replaced driveForward
        sleep(500);
        intake.setPower(0);

        PID_backward(29, 0.75);      // Replaced driveBackward
        sleep(500);

        PID_rotateLeft(45, 0.45);    // Replaced rotateLeft
        ty = result.getTy();
        sorter(motif_pattern, getdistance(ty));

        PID_rotateRight(45, 0.45);   // Replaced rotateRight
        sleep(500);

        lateralRight(23, 0.75);      // Replaced strafeRight
        sleep(500);
